...
# DATA_PATH = 'src/kaggle/Wine.csv'
# data = pd.read_csv(DATA_PATH, sep=',', header=None, names=['Wine', 'Alcohol', 'Malic.acid', 'Ash', 'Acl',
...
# x, standardizer, df_wine = load_data(DATA_PATH)
def load_data_not_pd(x):
    standardizer = preprocessing.StandardScaler()
    x = standardizer.fit_transform(x)
...
class DataBuilderRocket(Dataset):
    def __init__(self, data):
        self.x, self.standardizer = load_data_not_pd(data)
        self.x = numpyToTensor(self.x)
        self.len = self.x.shape[0]
...
# standardizer = DataBuilder.standardizer
# model.eval()
# test_loss = 0
...
# ax.scatter(xdata, ydata, zdata, c=winetype)
# plt.show()
