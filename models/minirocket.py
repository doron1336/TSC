...
def _fit_dilations(input_length, num_features, max_dilations_per_kernel, kernel_length):
...
def fit(X, num_features = 10000, max_dilations_per_kernel = 32, y=None):
    ...
    if input_length < 9:
        raise ValueError(
            (
                f"n_timepoints must be >= 9, but found {input_length};"
    ...
    dilations, num_features_per_dilation = _fit_dilations(input_length, num_features, max_dilations_per_kernel, kernel_length)
