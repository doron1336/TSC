...
def indicesNotSelected(selectedIndices, numSamples):
    temp = []
...
def GetHandMovementDATA_randomTest(precentOfTest):
    ...
    numRows = cyl_ch1_df.shape[0]
    test_indices = np.random.choice(numRows, int(precentOfTest*numRows), replace=False)
    train_indices = indicesNotSelected(test_indices, numRows)
    ...
    cyl_final['label'] = np.repeat([1], 30)
    # cyl_ch1_df['label'] = label
    train_cyl = cyl_final.iloc[train_indices]
    test_cyl = cyl_final.iloc[test_indices]
    ...
    hook_final['label'] = np.repeat([2], 30)
    # hook_ch1_df['label'] = label
    train_hook = hook_final.iloc[train_indices]
    test_hook = hook_final.iloc[test_indices]

    tip_ch1_df = pd.DataFrame(mat['tip_ch1'])
    tip_ch2_df = pd.DataFrame(mat['tip_ch2'])
    tip_final = pd.concat([tip_ch1_df, tip_ch2_df], axis=1)
    tip_final['label'] = np.repeat([3], 30)
    # tip_ch1_df['label'] = label
    train_tip = tip_final.iloc[train_indices]
    test_tip = tip_final.iloc[test_indices]
    ...
    palm_final['label'] =  np.repeat([4], 30)
    # palm_ch1_df['label'] = label
    train_palm = palm_final.iloc[train_indices]
    test_palm = palm_final.iloc[test_indices]
    ...
    spher_final['label'] = np.repeat([5], 30)
    # spher_ch1_df['label'] = label
    train_spher = spher_final.iloc[train_indices]
    test_spher = spher_final.iloc[test_indices]

    lat_ch1_df = pd.DataFrame(mat['lat_ch1'])
    lat_ch2_df = pd.DataFrame(mat['lat_ch2'])
    lat_final = pd.concat([lat_ch1_df, lat_ch2_df], axis=1)
    lat_final['label'] = np.repeat([6], 30)
    # lat_ch1_df['label'] = label
    train_lat = lat_final.iloc[train_indices]
    test_lat = lat_final.iloc[test_indices]
